"""
Unbounded Knapsack problem using dp
(Unbounded means all the given weights are available in infinite quantity)
Given weights and their corresponding values,
We fill knapsack of capacity W to obtain maximum possible value(or profit).
We can pick same weight more than once.
N: Number of (items)weight elements
W: Capacity of knapsack
Time Complexity: O(N*W)
(Optimizing knapsack at capacities from 0 to W gradually using all N items)
...
def unbounded_knapsack(capacity, weights, values):
    ...
    for current_capacity in range(capacity + 1):
        ...
        for i in range(items):
            ...
            # If the weight of item is less then current_capacity,
            # it can be used in knapsack
            if (weights[i] <= current_capacity):
                ...
                # Current item is utilised, so knapsack value for
                # current_capacity changes to value of current item + knapsack
                # value when capacity is current_capacity-weight of the item
