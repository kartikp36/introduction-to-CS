"""
Knapsack 0-1 problem using dp
(0-1 means we either choose it or we don't, no fractions)
Given weights and their corresponding values,
We fill knapsack of capacity W to obtain maximum possible value
in bottom-up manner.
...
def knapsack(capacity, weights, values):
    ...
    for i in range(items + 1):
        ...
        for j in range(capacity + 1):
            ...
            # Weight of item is greater than current capacity,
            # so we cannot use it. So knapsack value here is
            # the best we can do without this item ie. with the (i-1)th item
            elif weights[i-1] > j:
                knapsack_table[i][j] = knapsack_table[i-1][j]

            # Weight of item is less than current capacity,
            # so we can use it to optimize knapsack
            else:
                knapsack_table[i][j] = max(
                    # If i_th item is included,
                    # knapsack value is equal to it's value + best we can do
                    # (ie. value of knapsack_table):
