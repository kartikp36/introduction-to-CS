...
Given two strings, return the minimum number of operations required
to convert one word to other. You can apply the following operatiions:
...
def editDist(first_string, second_string):
    ...
    # Looping and comparing each letter of first_string
    # with each of second_string
    for first_sub_string in range(first_length + 1):
        for second_sub_string in range(second_length + 1):

            if first_sub_string == 0:
                # first_sub_string is empty,
                # so insert all charecters of second_sub_string
                distance[first_sub_string][second_sub_string] = second_sub_string

            elif second_sub_string == 0:
                # second_sub_string is empty,
                # so insert all charecters of first_sub_string
                ...
                # If last charecters of both substring are same,
                # no operation required here. It's value will be equal to
                # edit distance of substring without this same charecters
                # ie. Diagonal value
                ...
                # If last charecters of both substring are not same,
                # we can either insert, remove or replace a charecter
                # We choose minimum possible edit distance of the substrings
                # before performing operation and we add 1 to edit distance
                # for the one operation we need to perform to convert
